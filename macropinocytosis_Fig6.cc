...
double func_vamp(double *AA,double *II,double *p, int i, int j,int k, double VOL){
  ...
  return (eta*(d2th(p,i,j,k)-16.0*p[i*L*Lz+j*Lz+k]*(1.0-p[i*L*Lz+j*Lz+k])*(1.0-2.0*p[i*L*Lz+j*Lz+k])/(ep*ep))+(-Mem*(VOL-V0)+F0*Fpoly(AA[i*L*Lz+j*Lz+k],II[i*L*Lz+j*Lz+k],K,Kd) )*d1th_abs(p,i,j,k))/tau;

}

//functions for calulating reaction-diffusion
double func_A(double *AA,double *II,double AT, double inta,int i, int j,int k){
  return  AA[i*L*Lz+j*Lz+k]*AA[i*L*Lz+j*Lz+k]*(AT-inta)/(1.0+II[i*L*Lz+j*Lz+k])-AA[i*L*Lz+j*Lz+k];
